        ...
        <div>
            <form onSubmit={handleSubmit}  >
                ...
                <div className="relative">
                    ...
                    <input onClick={e => fetchSearchData(e.target.value)} value={searchTerm} onChange={e => fetchSearchData(e.target.value)} type="search" id="default-search" className="block w-full p-4 pl-10 text-sm text-gray-900 border border-gray-300  bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white  " placeholder="Search" required />
                    <button type="submit" className="text-white absolute right-2 md:right-44 lg:right-48 xl:right-52 2xl:right-64  bottom-2.5 font-medium rounded-lg text-sm px-4 py-2 my-bg-color"> <svg className="w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                        <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z" />
                    </svg></button>
                </div>
            </form>
